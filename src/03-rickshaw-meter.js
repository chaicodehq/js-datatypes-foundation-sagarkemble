...
export function parseFare(fareString) {
  if (typeof fareString != "string" || parseFloat(fareString) === NaN)
    return -1;
...
export function findCheapestAndCostliest(...fares) {
  ...
  const filteredMap = fares.filter((e) => {
    if (typeof e == "number") return e;
  });
  if (filteredMap.length == 0) return null;
  const cheapest = Math.min(...filteredMap);
  const costliest = Math.max(...filteredMap);
  return { cheapest, costliest };
}
